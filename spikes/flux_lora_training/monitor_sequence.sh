#!/usr/bin/env bash
# Check sequence generation progress on pod.
# Usage: bash spikes/flux_lora_training/monitor_sequence.sh [--last N]
set -euo pipefail
...
EXPECT_SCRIPT=$(mktemp /tmp/mon_seq_XXXXXX.expect)
...
send "tmux has-session -t sequence 2>/dev/null && echo GEN:running || echo GEN:stopped; echo __M1__\r"
expect { "__M1__" {} timeout { puts "timeout M1"; exit 1 } }
expect -re {#\s}

send "echo '=== last \$n log lines ==='; tail -n \$n /workspace/sequence.log 2>/dev/null || echo '(no log yet)'; echo __M2__\r"
